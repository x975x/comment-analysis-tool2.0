...
with st.sidebar:
    ...
    # 导航在脚本末尾渲染（确保分析后 session_state 已更新）
...
if "LDA" in mode:
    ...
    if st.session_state['lda_results'] is not None:
        with st.expander("✏️ 调整停用词 / 自定义词典（可边看结果边修改，无需重传文件）"):
            st.markdown(
                "✏️ 直接在下方文本框中增删词条，完成后点击「保存并重新分析」。  \n"
            ...
            if save_submitted:
                ...
                _save_editor_persist(new_stop, new_dict)
...
# ════════════════════════════════════════════════════════════
#  侧边栏导航（脚本末尾渲染，确保分析后 session_state 已更新）
# ════════════════════════════════════════════════════════════
with st.sidebar:
    st.divider()
    st.markdown("#### 📌 页面导航")
