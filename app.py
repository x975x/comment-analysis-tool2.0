...
def _nav_link(href: str, label: str) -> str:
    ...
    return (
        f'<a href="{href}" style="display:flex;align-items:center;'
        f'text-decoration:none;color:#262730;padding:5px 2px;font-size:0.95rem;">'
...
if "LDA" in mode:
    ...
    # ── 在线调整停用词 / 词典 ────────────────────────────────
    # ⚠️ 关键：此 expander 必须在 if run_lda or editor_submitted 之前渲染，
    #    才能让「保存并重新分析」的返回值在同一次脚本执行中触发分析，
    #    无需 st.rerun()，彻底避免双次渲染导致的分析失败问题。
    _anchor("lda-editor")
    editor_submitted = False
    if st.session_state['lda_results'] is not None:
        with st.expander("✏️ 调整停用词 / 自定义词典（可边看结果边修改，无需重传 CSV）"):
            st.caption(
                "直接在下方文本框中增删词条，完成后点击「保存并重新分析」。"
                "评论数据已缓存，无需重新上传。"
            )
            ...
            if st.button(
                ...
                # 立即写入 session_state，下方分析逻辑即可读取最新值
                st.session_state['lda_stop_text'] = new_stop
                st.session_state['lda_dict_text'] = new_dict
                editor_submitted = True   # 在同一次渲染中触发分析，无需 st.rerun()

    # ── 运行分析（首次点击「开始分析」或「保存并重新分析」）────────────
    if run_lda or editor_submitted:
        ...
        if run_lda:
            ...
            # 若本次上传了新的停用词 / 词典文件，读取并存入 session_state
        ...
        else:   # editor_submitted：CSV 来自缓存，无需重传
